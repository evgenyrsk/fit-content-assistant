...
- source assessment может выполнить только один наблюдаемый repair-проход после ошибки strict contract или provenance; repair получает безопасный идентификатор нарушенного раздела и точный design-specific список проверок, provider errors не повторяются, а невалидный второй ответ закрывает gate;
