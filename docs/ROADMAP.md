...
- [x] Выделить достаточный token budget для строгих assessment-контрактов и показывать безопасную причину fail-closed отказа.
  ...
  - [x] Передавать bounded repair точный runtime issue и design-specific список проверок после продовой регрессии сложных full-text источников.
...
- [x] Показывать отдельно provenance-linked выборку, численность и группы в кратком разборе статьи, не заполняя пропуски догадками.
