...
Без LLM владелец может собрать claim вручную только из full-text фрагментов источников, уже включённых через Source Inbox. Новая версия всегда начинается как `needs_review`; отдельный человеческий gate требует проверки provenance, scope и противоречащих данных. Раздел контента читает реальный D1-архив и показывает честное пустое состояние вместо демо-записей.
