...
export function TopicBankPanel() {
  ...
  return <section className="content-workbench-panel topic-bank-panel"><header><div><p className="overline">TOPIC INTELLIGENCE</p>
    ...
    <div className="topic-bank-grid"><section><div className="subsection-heading"><FlaskConical aria-hidden="true" /><div><h3>Автоподбор без LLM</h3>
      ...
      <form className="compact-form" onSubmit={(event) => void addTopic(event)}><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Тема" />
        <textarea rows={3} value={angle} onChange={(event) => setAngle(event.target.value)} placeholder="Угол подачи" />
        <select value={format} onChange={(event) => setFormat(event.target.value as ContentFormat)}><option value="threads">Threads</option><option value="reels">Reels</option>
    ...
    <section className="personal-notes"><div className="subsection-heading"><NotebookPen aria-hidden="true" /><div><h3>Личные заметки</h3>
      <p>Опыт можно использовать в повествовании, но никогда как proof.</p></div></div><div className="notes-layout"><form className="compact-form" onSubmit={(event) => void addNote(event)}>
        <select value={noteKind} onChange={(event) => setNoteKind(event.target.value as UserNote['kind'])}><option value="personal_experience">Личный опыт</option><option value="observation">Наблюдение</option><option value="idea">Идея</option></select>
        <input value={noteTopic} onChange={(event) => setNoteTopic(event.target.value)} placeholder="Тема заметки" />
        <textarea rows={4} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Текст" /><button disabled={topicBank.saving}>Сохранить заметку</button></form>
