...
import { ArrowLeft, ArrowRight, Check, FlaskConical, Play, X } from 'lucide-react';
import { evidenceFlowSteps, stageGuardrails } from './evidence-flow-demo-data';
import { EvidenceFlowPanel } from './evidence-flow-panels';

export function EvidenceFlowDemo({ onLiveRun }: { onLiveRun: () => void }) {
  ...
  return <section className="evidence-flow-demo" id="evidence-flow-demo" aria-labelledby="evidence-flow-title">
    <header className="evidence-flow-header"><div><p className="overline">END-TO-END CONTROLLED DEMO</p><h2 id="evidence-flow-title">От научного вопроса до контента, которому можно верить</h2><p>Источники реальные. Оценка и контент показаны как воспроизводимый демо-проход и не записываются в базу знаний.</p></div><div className="evidence-flow-header-actions"><span><FlaskConical aria-hidden="true" />Тема: отказ и гипертрофия</span><button type="button" onClick={onLiveRun}><Play aria-hidden="true" />Запустить свой вопрос</button></div></header>
