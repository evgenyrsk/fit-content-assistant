...
import { ClaimDraftReview } from './claim-draft-review';

const confidenceLabels = {
  high: 'высокая', moderate: 'умеренная', low: 'низкая', insufficient: 'недостаточно данных',
} as const;

export function ClaimDraftResult({ response, onContentFormat }: {
  response: ClaimSynthesisResponse; onContentFormat: (format: ContentFormat, claimVersionId: string) => void;
}) {
  ...
  return <section className="claim-draft-result"><header><div><span><FileCheck2 aria-hidden="true" />CLAIM DRAFT</span><h3>{claim.statement}</h3></div><em>нужна проверка</em></header>
    ...
    <ClaimDraftReview versionId={response.saved?.versionId ?? null} onContentFormat={onContentFormat} />
  </section>;
}
