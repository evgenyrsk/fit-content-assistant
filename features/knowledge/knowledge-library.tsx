  ...
  return (
    <div className="library-panel">
      <div className="library-toolbar">
        ...
        <label><Search aria-hidden="true" /><input value={filters.query} onChange={(event) => filters.setQuery(event.target.value)} placeholder="Найти claim, тему или ограничение" /></label>
      </div>
      <div className="filter-row knowledge-filter-row">
        <div className="filter-group"><span>Уверенность</span><select value={filters.confidence} onChange={(event) => filters.setConfidence(event.target.value)}><option value="all">Все уровни</option><option value="high">Высокая</option><option value="moderate">Умеренная</option><option value="low">Низкая</option><option value="insufficient">Недостаточно</option></select></div>
        <div className="filter-group"><span>Статус</span><select value={filters.status} onChange={(event) => filters.setStatus(event.target.value)}><option value="all">Все статусы</option><option value="approved">Подтверждено</option><option value="needs_review">Нужна проверка</option><option value="rejected">Отклонено</option><option value="superseded">Заменено</option></select></div>
        <div className="filter-group"><span>Свежесть</span><select value={filters.freshness} onChange={(event) => filters.setFreshness(event.target.value)}><option value="all">Любая</option><option value="current">Актуальные</option><option value="due">Перепроверить</option></select></div>
        <div className="filter-group"><span>Источник</span><select value={filters.sourceType} onChange={(event) => filters.setSourceType(event.target.value)}><option value="all">Все типы</option>{filters.sourceTypes.map((value) => <option key={value}>{value}</option>)}</select></div>
        <div className="filter-group"><span>Популяция</span><select value={filters.population} onChange={(event) => filters.setPopulation(event.target.value)}><option value="all">Все популяции</option>{filters.populations.map((value) => <option key={value}>{value}</option>)}</select></div>
